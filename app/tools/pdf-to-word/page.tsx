...
function PdfToWordContent() {
  ...
  useEffect(() => {
    document.title = `${t("toolPages.convertToWord")} - PDFCraft`;
  }, [t]);
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/tools/advanced", {
        ...
        body: JSON.stringify({ fileId: uploadedFile.id, conversion: "pdfToWord" }),
      });
      ...
      complete(json.data.downloadUrl, "converted.docx");
    } catch (err) {
      fail(err instanceof Error ? err.message : t("processing.failed"));
    }
  }, [uploadedFile, startProcessing, complete, fail, t]);
