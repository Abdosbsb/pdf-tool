...
function RotatePdfContent() {
  ...
  const handleRotate = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/tools/rotate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileId: uploadedFile.id, degrees }),
      });
      ...
      complete(json.data.downloadUrl, "rotated.pdf");
    } catch (err) {
      fail(err instanceof Error ? err.message : t("processing.failed"));
    }
  }, [uploadedFile, degrees, startProcessing, complete, fail, t]);
