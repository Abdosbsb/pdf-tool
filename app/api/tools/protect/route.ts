...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    const job = createJob("protect-pdf", [fileId], buffer.length);
    ...
    const result = await processor.addPassword(buf, password);
    const resultBuffer = Buffer.from(result);

    const outputMeta = createFileMeta("protected.pdf", resultBuffer.length, "application/pdf");
    ...
    return NextResponse.json(
      { success: false, error: { code: "PROTECT_FAILED", message: "Failed to protect PDF" } },
      { status: 500 }
    );
  }
}
